...
namespace re
{
    geom::Matrix44 Camera::matrix()
    {
        return geom::create_scale_matrix_44({1.f, 1.f, 0.001f}) * geom::create_rotation_matrix_from_quaternion(orientation.inverse()) * geom::create_translation_matrix_44(-pos);
    }
    ...
    void Scene::editor_ui()
    {
        if(ImGui::Begin("Scene"))
        {
            ...
            {
                ImGui::PushID(&entity);
                ImGui::InputFloat3("Pos", &entity.pos.x);
                ImGui::PopID();
            }
        }
        ImGui::End();
